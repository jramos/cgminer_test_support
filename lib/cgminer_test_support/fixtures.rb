...
module CgminerTestSupport
  module Fixtures
    ...
    # Successful restart. MSG_RESTART = 42, Msg = "Restart returned".
    # Promoted from cgminer_manager/dev/screenshots/fake_cgminer_fleet.rb
    # so consumers using DEFAULT can exercise the scheduler /
    # admin-tab restart path without falling through to "Invalid
    # command" (which is what the #2.2 smoke test surfaced).
    RESTART_OK = <<~JSON.tr("\n", '')
      {"STATUS":[{"STATUS":"S","When":#{WHEN},"Code":42,"Msg":"Restart returned","Description":"cgminer 4.11.1"}],"id":1}
    JSON

    # Successful quit. MSG_BYE = 42 (shared with restart per cgminer's codes[]).
    QUIT_OK = <<~JSON.tr("\n", '')
      {"STATUS":[{"STATUS":"S","When":#{WHEN},"Code":42,"Msg":"Bye","Description":"cgminer 4.11.1"}],"id":1}
    JSON

    # Successful zero. MSG_ZERSUM = 72.
    ZERO_OK = <<~JSON.tr("\n", '')
      {"STATUS":[{"STATUS":"S","When":#{WHEN},"Code":72,"Msg":"Statistics zeroed","Description":"cgminer 4.11.1"}],"id":1}
    JSON

    # Successful save. MSG_SAVED = 20.
    SAVE_OK = <<~JSON.tr("\n", '')
      {"STATUS":[{"STATUS":"S","When":#{WHEN},"Code":20,"Msg":"Configuration saved","Description":"cgminer 4.11.1"}],"id":1}
    JSON
